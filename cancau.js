...
${hasAllShards(user.data.shards) ? "\n🔥 Bạn đủ 4 mảnh! Dùng **cancau ghep** để ghép Cần Câu Vô Hạn!" : ""}`
        );
    }
    ...
    if (action === "ghep") {
        ...
        if (success) {
            ...
            return send("🔥 **GHÉP THÀNH CÔNG!** Bạn nhận được **Cần Câu Vô Hạn**!");
        } else {
            await Users.setData(senderID, user);
            return send("💥 Ghép thất bại! Toàn bộ mảnh đã bị mất.");
        }
    }
    ...
    if (!action) {
        ...
        const available = Object.keys(canCauList)
            ...
            .map(key => `${canCauList[key].name} — Giá: ${canCauList[key].price.toLocaleString()}$ — Rate: ${canCauList[key].rate}`)
            .join("\n") || "Không còn cần câu nào để mua";

        return send(
`🎣 **Cần câu đang trang bị:** ${equipped}

🪝 **Các cần câu bạn sở hữu:** ${owned}

💰 **Các cần câu có thể mua:**
${available}

💡 Hướng dẫn sử dụng:
- Dùng **cancau mua [loại]** để mua cần câu
- Dùng **cancau trangbi [loại]** để trang bị cần câu
- Dùng **cancau ghep** nếu bạn đủ 4 mảnh Vô Hạn
- Dùng **cancau shard** để xem số lượng mảnh`
        );
    }
    ...
    if (action === "mua") {
        ...
        if (money < canCauList[type].price) return send(`❌ Không đủ tiền! Giá: ${canCauList[type].price.toLocaleString()}$`);
    ...
    if (action === "trangbi") {
        ...
        await Users.setData(senderID, user);
        return send(`✅ Bạn đã trang bị **${canCauList[type].name}** (Rate: ${canCauList[type].rate})`);
    }
};

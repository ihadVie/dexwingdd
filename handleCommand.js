module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
    ...
    // ----- WHITELIST SETUP (load file) -----
    const whitelistPath = path.join(__dirname, "groupWhitelist.json");
    if (!fs.existsSync(whitelistPath)) {
      fs.writeFileSync(whitelistPath, JSON.stringify({}, null, 4));
    }
    let whitelistData;
    try {
      whitelistData = JSON.parse(fs.readFileSync(whitelistPath));
    } catch (err) {
      whitelistData = {};
      fs.writeFileSync(whitelistPath, JSON.stringify({}, null, 4));
    }
    ...
    if (!whitelistData[threadID]) whitelistData[threadID] = { allowed: [] };

    // === TÙY CHỌN: tự whitelist cho một vài group khi cần ===
    // Ví dụ: nhóm bạn yêu cầu chỉ cho phép 'pay' (ID bạn đưa trước đó)
    // Nếu muốn tắt, xóa hoặc chỉnh id/command ở dưới
    if (threadID === "8919919934793676") {
      whitelistData[threadID].allowed = ["pay"];
    }

    // lưu file (an toàn)
    try { fs.writeFileSync(whitelistPath, JSON.stringify(whitelistData, null, 4)); } catch (e) {}

    // tiếp tục phần xử lý command
    ...
    // ========== KIỂM TRA WHITELIST SAU KHI CÓ commandName ==========
    try {
      const allowedList = (whitelistData[threadID] && Array.isArray(whitelistData[threadID].allowed)) ? whitelistData[threadID].allowed : [];

      if (allowedList.length > 0) {
        // ADMINBOT & NDH bypass whitelist
        if (!ADMINBOT.includes(senderID) && !NDH.includes(senderID)) {
          if (!allowedList.includes(commandName)) {
            return api.sendMessage(
              `⚠️ Hãy tham gia nhóm 🪽 DexWing để sử dụng bot:\n• box 1: https://m.me/j/AbbEEfHSEJWbeB2o\n• Box 2: https://m.me/j/AbbskUZCLQwhdj0P/`,
              threadID,
              messageID
            );
          }
      ...
      // lỗi đọc whitelist — tiếp tục để bot không chết
      logger("Lỗi khi kiểm tra whitelist: " + e, "ERROR");
    }
      ...
      if (checker.bestMatch.rating >= 0.5) command = client.commands.get(checker.bestMatch.target);
        ...
        let callback = function () {
          api.sendMessage({
            ...
            try { fs.unlinkSync(__dirname + `/1.png`); } catch (e) { }
            try { fs.unlinkSync(__dirname + `/2.png`); } catch (e) { }
          }, event.messageID);
        };

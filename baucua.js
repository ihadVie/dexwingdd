...
var full = [];
    ...
    module.exports.run = async function({ api, event, args, Currencies }) { var out = (msg) => api.sendMessage(msg,event.threadID, event.messageID);
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                var moneyBet = parseBetAmount(args[1]);
                ...
                if (moneyBet > moneyUser) return api.sendMessage("𝐒𝐨̂́ 𝐭𝐢𝐞̂̀𝐧 𝐛𝐚̣𝐧 𝐝𝐚̣𝐭 𝐥𝐨̛́𝐧 𝐡𝐨̛𝐧 𝐬𝐨̂́ 𝐝𝐮̛ 𝐜𝐮̉𝐚 𝐛𝐚̣𝐧!", event.threadID, event.messageID);
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            await Currencies.decreaseMoney(event.senderID, moneyBet); console.log('s4')
            return api.sendMessage({body:`𝐊𝐞̂́𝐭 𝐪𝐮𝐚̉: ${full.join(" | ")}\n𝐁𝐚̣𝐧 𝐝𝐚̃ 𝐛𝐢̣ 𝐭𝐡𝐮𝐚 𝐯𝐚̀ 𝐭𝐫𝐮̛̀  ${moneyBet}$ 💸\n𝐯𝐢̀ 𝐤𝐡𝐨̂𝐧𝐠 𝐜𝐨́ 𝐜𝐨𝐧 ${icon}`,attachment: listimg},event.threadID, event.messageID);
        }
            } ,event.messageID);
    };
